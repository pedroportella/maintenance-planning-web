...
This repository hosts a React and Next.js workbench for planners who review API-backed work-order recommendations, schedule posture and decision history. The current implementation includes repo guardrails, a local App Router shell, a typed service boundary that can run in deterministic mock mode or explicit backend mode, and a neutral visual foundation for the workbench.
